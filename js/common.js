document.addEventListener("DOMContentLoaded", (event) => {
  ...
  async function initPageChanger() {
    ...
    const PAGE_LINKS = pageNames
      .map((name) => {
        // Локальная разработка
        if (isLocalhost) {
          return name === "" ? "/index.html" : `/${name}.html`;
        }

        // GitHub Pages
        if (isGitHubPages) {
          return name === "" ? "/agoy/index.html" : `/agoy/${name}.html`;
        }
    ...
    async function fetchLinks() {
      try {
        ...
        const response = await fetch(
          "https://grand-hotel-agoy.ru/api/get_menu.php"
        );
    ...
    function handleDesktopScroll(e) {
      ...
      console.log(
        `Scroll: deltaY=${e.deltaY}, top=${isTop}, bottom=${isBottom}`
      );

      if (scrollTimeout) clearTimeout(scrollTimeout);

      if (
        (e.deltaY > 0 && isBottom && canGoNext()) ||
    ...
    function handleTouchMove(e) {
      ...
      console.log(
        ...
        deltaY < -20 && isBottom && canGoNext() && !isSliderCanVertical.down
      );
      console.log(
        "if(deltaY > 20 && isTop && canGoPrev() && !isSliderCanVertical.up):",
  ...
  function headerHide() {
    ...
    window.addEventListener("scroll", function () {
      ...
      if (
        ...
        Math.abs(scrollTop - lastScrollTop) > scrollDelta
      ) {
  ...
  function initSlides() {
    ...
    if (bigSlider != undefined) {
      let bigSliderSwiper = new Swiper(".big-slider", {
        ...
        on: {
          slideChange: function () {
            ...
            console.log(
              "isSliderCanVertical.down swiper: ",
              isSliderCanVertical.down
            );
            console.log(
              "isSliderCanVertical.up swiper: ",
              isSliderCanVertical.up
            );
      ...
      console.log(
        'document.querySelectorAll(".big-slider .swiper-slide").length',
        document.querySelectorAll(".big-slider .swiper-slide").length
      );
      if (
        window.innerWidth <= 1024 &&
        document.querySelectorAll(".big-slider .swiper-slide").length > 1
      ) {
        isSliderCanVertical.up = true;
        isSliderCanVertical.down = false;
      } else {
  ...
  function initForms() {
    ...
    document
      .querySelectorAll('[data-hystmodal="#modal-form"]')
      .forEach((caller) => {
    ...
    // Новая инициализация
    const popupFormCallers = document.querySelectorAll(
      '[data-hystmodal="#modal-form"]'
    );
    ...
    if (popupForm) {
      ...
      popupFormCallers.forEach((caller) => {
        const clickHandler = () => {
          popupForm.querySelector(".modal-form__title h2").innerText =
            caller.dataset.formTitle || "Обратная связь";
          popupForm.querySelector(".modal-form__desc p").innerText =
            caller.dataset.formDesc || "";

          if (popupForm.querySelector("[data-form-name]")) {
            popupForm.querySelector("[data-form-name]").value =
              caller.dataset.formName || "Обратная связь";
          }
        };
  ...
  function initAdapt() {
    ...
    elements.forEach((element) => {
      ...
      const [target, breakpoint, order] = element
        .getAttribute("data-adapt")
        .split(",")
        .map((item) => item.trim());
      ...
      if (window.innerWidth <= parseInt(breakpoint)) {
        ...
        const originalContainerAttr = element.getAttribute(
  ...
  function initTableTabs() {
    document.querySelectorAll(".invest__tab").forEach((tab) => {
      tab.addEventListener("click", function (e) {
        ...
        document
          .querySelectorAll(".invest__tab")
          .forEach((t) => t.classList.remove("invest__tab--active"));
        // Добавляем активный класс текущему табу
        this.classList.add("invest__tab--active");

        // Скрываем все таблицы
        document
          .querySelectorAll(".invest__table table")
          .forEach((table) => table.classList.remove("invest__table--active"));
        // Показываем таблицу с соответствующим data-table-id
        const tableId = this.getAttribute("data-table");
        document
          .querySelector(`.invest__table table[data-table-id="${tableId}"]`)
          .classList.add("invest__table--active");
      });
    });
  }
  ...
  function initBbf() {
    ...
    bbfs.forEach((bbf) => {
      ...
      button.addEventListener("click", () => {
        bbfModal.classList.toggle("bbf-modal-active");
        if (bbfModal.classList.contains("bbf-modal-active")) {
          body.style.overflow = "hidden";
        } else {
          body.style.overflow = "auto";
...
function initAspectRatio() {
  ...
  function updateAspectRatio() {
    ...
    items.forEach((item) => {
      let aspectRatio;
      if (
        item.dataset.tabletAspectRatio &&
        window_width <= 1024 &&
        window_width > 766
      ) {
